...
static int uc1698fb_proc_close(struct inode *inode, struct file *file)
{
...
static int uc1698fb_proc_open(struct inode *inode, struct file *file)
{
...
static int uc1698fb_chip_probe(void)
{
    ...
    HNOS_DEBUG_INFO("No UC1698U chip found!\n"); 
    return 0; 
}
...
#else
static ssize_t uc1698fb_proc_read(struct file *file, char __user * page, size_t count,
		       loff_t * ppos)
{
	static int len=0;
	

	if (lcd_is_first_read)
  	{
		lcd_is_first_read=0;

		len += sprintf(page+len, "lcd_bias	:\t0x%2x\n", uc1698fb_bias_data_get());
		len += sprintf(page+len, "temp_comp	:\t0x%2x\n", uc1698fb_temp_comp_get());
		len += sprintf(page+len, "bias_ratio :\t0x%2x\n", uc1698fb_bias_ratio_get());
...
static int uc1698fb_release(struct inode *inode, struct file *filp)
{
 ...
 static struct file_operations file_ops = {
	.open=uc1698fb_proc_open,
	.release=uc1698fb_proc_close,
