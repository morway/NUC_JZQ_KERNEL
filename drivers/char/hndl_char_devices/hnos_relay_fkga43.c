...
{      
	int ret = 0;
		...
		ret = lv595_dev->iomem->write_byte(lv595_dev->iomem, ledvalue);
...
static int __init lv595_init(void)
{
	...
	lv595_dev->iomem = iomem_object_get(RELAY_CS, 0);
	if (!lv595_dev->iomem) {
		printk(KERN_ERR "%s: can't get iomem (phy %08x).\n", __FUNCTION__,
				(unsigned int) RELAY_CS);
		result = -1;
		goto iomem_fail;
	}
	...
	HNOS_DEBUG_INFO("HNOS_FKGA43_LED registered.\n");
	return 0;
