...
{
	//printk("---rx8025_read_reg \n");
    int ret=1;// = i2c_smbus_read_byte_data(client, (number << 4) | 0x08);
	
    i2c_smbus_write_byte(client, number);// (<< 4) | 0x08);
...
#else
   //i2c_smbus_write_byte(client, number);// << 4) | 0x08);
   //*values =i2c_smbus_read_byte(client);
   for(i=0;i<length;i++)
   	{
   	  i2c_smbus_write_byte(client, ((number+i)));//<< 4) | 0x08);
      *values =i2c_smbus_read_byte(client);
	    values++;
   	}
	 return 0;
...
{
	//printk("---rx8025_write_reg \n");
#if 1
	int ret = i2c_smbus_write_byte_data(client, number, value);  // number << 4
	...
	return ret;
#else
  i2c_smbus_write_byte(client, ((number)));
...
static int rx8025_get_time(struct device *dev, struct rtc_time *dt)
{
 ...
 // printk("rx8025_get_time \n");
  //rx8025_read_reg(rx8025->client, RX8025_REG_SEC, date);
	...
	*/
	...
	if(first_run)
		{
	printk("enter rx8025_second_read!\n");
	rx8025_write_reg(rx8025->client, 0x0d,0x00);
	rx8025_write_reg(rx8025->client, 0x0e,0x00);
	rx8025_write_reg(rx8025->client, 0x0f,0x60);
	first_run=0;
		}
 ...
 */
	...
	if(first_run)
		{
	printk("enter rx8025_second_write!\n");
	rx8025_write_reg(rx8025->client, 0x0d,0x00);
	rx8025_write_reg(rx8025->client, 0x0e,0x00);
	rx8025_write_reg(rx8025->client, 0x0f,0x60);
	first_run=0;
...
//	rx8025_write_regs(rx8025->client, 0x0d, 1, 0x00);
//	rx8025_write_regs(rx8025->client, 0x0e, 1, 0x00);
//	if(rx8025->client==NULL)
//		printk("rx8025_init_client:NULL\n");
//	else
//	rx8025_write_reg(rx8025->client, 0x0f,0x60);
	...
	if (*need_reset || need_clear) {
		...
		err = rx8025_write_reg(client, RX8025_REG_CTRL2, 0); //ctrl2
	}
out:
